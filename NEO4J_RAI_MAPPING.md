...
| `CodeResolution` | deterministic resolution-context hash | source object/DV-46 row token/field role/raw code/method, DV-10/DV-11, reference lineage | Stable result exists at zero/one/many candidates, including retained plan rows without source IDs. Concept-valued link exists only for exact cardinality one. |
| `CodeResolutionCandidate` | `(resolution_id, candidate_id)` | one CodeResolution and one Airport/Airline candidate | Zero-to-many from resolution result; avoids nullable candidate identity for unresolved inputs. |
...
| `ScheduleAmendmentEvidence` | DV-35 candidate, DV-36 group, DV-37 member | DV-18/DV-19, DV-34 comparison, removed/added schedules, typed candidate signature and evidence | Many-to-many evidence; ambiguous groups have all members and no chosen pair. |
...
| `FulfillmentCandidateEvidence` | DV-40 candidate, DV-41 group, DV-42 member | DV-23, DV-38 carrier/date/ordered stopover compatibility | Separate from confirmed fulfillment; unique-per-actual candidates may preserve one-to-many stopovers, while competing-plan groups remain unlinked. |
...
| `QueryExecutionContext` | query invocation ID | DV-29 carrier role, DV-30 knowledge date, DV-31 operating date | Capacity/frequency requires all three; missing input is a clarification/error state. |
...
|---|---|---|---|
| `(AIRCRAFT)-[:HAS]->(AIRCRAFT_STATE)` | Aircraft to `AircraftStateDailyAssignment`; audit inverse also exposed | aircraft, state assignment, DV-06/DV-07, event lineage | One-to-many association. Daily parity plus separate audit history. |
| `(AIRCRAFT)-[:CONFORMED]->(AIRCRAFT_TYPE)` | `AircraftTypeDailyAssignment` links Aircraft to optional exact AircraftType | aircraft/type roles, DV-06/DV-07, AH/AC resolution/provenance | One-to-many over time; zero target in unresolved gap. Audit assignments preserve every event-level change. |
| `(AIRCRAFT)-[:EQUIPPED]->(ENGINE_TYPE)` | `EngineTypeDailyAssignment` links Aircraft to optional exact EngineType | aircraft/engine roles, DV-06/DV-07, count, DV-09, lineage | One-to-many over time; zero target in unresolved gap; no fabricated second type. |
| `(AIRCRAFT)-[:ASSIGNED]->(AIRCRAFT_STATUS)` | `AircraftStatusDailyAssignment`, plus audit assignment sequence | aircraft/status roles, DV-06/DV-07, AH-08/AH-12 | Daily validity for as-of; audit association for all reversion spells and same-day zero-day spell. |
...
| `(AIRLINE)-[:OPERATES]->(ROUTE_STATE)` | RouteState `operating_airline` exact link / inverse Relationship | explicitly labeled operating role; SS-05 and resolution | Zero-or-one exact link per state after cardinality proof; raw code/status otherwise. |
| `(AIRLINE)-[:COMMERCIALIZES]->(ROUTE_STATE)` | RouteState `marketing_airline` exact link / inverse Relationship | explicitly labeled marketing role; SS-04 and resolution | Zero-or-one exact link per state after cardinality proof; never merged with operating role. |
| `(ROUTE_STATE)-[:SCHEDULES]->(PASSENGER_FLIGHT)` | RouteState-to-PassengerFlight scheduling Relationship | exact schedule identity where available; passenger publish date in DV-14/DV-15; passenger operating date within inclusive SS-08/SS-09 and matching weekday; plan lineage | Potential one-to-many; no functional claim. Missing schedule identity or failed clock applicability remains unlinked/visible. |
...
No supplied edge is omitted. Validity and multi-attribute edge payloads are association properties,
not copied onto target identities. This keeps repeated assignments and concurrent states distinct.
