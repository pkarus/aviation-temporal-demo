...
| `CodeResolutionCode` (code grain) **and the row-scoped `CodeResolution` / `CodeResolutionCandidate`, both bound as of D-0027** | `(domain, method, raw_code)` | resolved identity, candidate count, resolution status, matched code systems, DV-10/DV-11 | Stable result exists at zero/one/many candidates. Concept-valued link exists only for exact cardinality one. **As built the grain is the distinct raw code, not the source row.** D-0019: the row-scoped `MODEL_INPUT.CODE_RESOLUTION` is 546,494 rows - 3.8 times the whole SOURCE layer - and link semantics only ever ask "does this raw code resolve to exactly one identity", which is a property of the code. The row-scoped table stays materialized and reachable from SQL so the NF-X01 truth table and any audit of a specific ambiguous code remain answerable. *Revised 2026-09-02 (TRUTH-01): this row's header read "(D-0019 supersedes the row-scoped `CodeResolution`)" and the cell said "**As built the grain is the distinct raw code, not the source row.**" D-0019's exclusion was provisional on a cost suspicion; D-0027 measured that cost at 0.18s against an 18s first query, refuted the premise, and reinstated the row grain. `CodeResolution` (`identify_by={"resolution_id": String}`) and `CodeResolutionCandidate` are bound concepts today at `core_reference.py:140-180`, and `CODE_RESOLUTION`, `CODE_RESOLUTION_CANDIDATE` and `CODE_RESOLUTION_INPUT` are three of the 37 declared sources. Neither grain supersedes the other: the code grain carries link semantics, the row grain carries provenance - which source object, row and field role mentioned a given raw code.* |
...
| `(ROUTE_STATE)-[:SCHEDULES]->(PASSENGER_FLIGHT)` | `RouteStateSchedules` / `RouteState.schedules`, a derived multi-valued `Relationship`, with the `.alt()` inverse `PassengerFlight.scheduled_by` (`computed_schedule.py`) | exact schedule identity where available; passenger publish date in DV-14/DV-15; passenger operating date within inclusive SS-08/SS-09 and matching weekday; plan lineage | Potential one-to-many; no functional claim. Missing schedule identity or failed clock applicability remains unlinked/visible. **Live population is 7,950 pairs** (7,950 distinct passenger flights, 2,000 distinct route states), measured 2026-09-02 by a SQL replica of the four-clause rule against `MODEL_INPUT`. `PASSENGER_FLIGHT_CANONICAL.schedule_key` is non-null on 7,951 of 19,446 canonical rows and all but one of those satisfies the full conjunction. No frozen `EXPECTED_ANSWERS.yaml` scenario reads a value through this relationship, so it is **populated but ungraded**: say "implemented and populated, not covered by a golden-question fixture". *Revised 2026-09-02 (TRUTH-01): this cell read "**Live population is zero pairs**... `schedule_key` is non-null on 1 of 19,996 rows, and that one key (`SYN-SK-HIST_700`) has no `ROUTE_STATE` at all... unexercised by the shipped fixture." That was measured before the D-0023/D-0024 enrichment raised schedule-key coverage on purpose, and it is now false by three orders of magnitude.* |
...
loaded, and is populated with 7,950 live pairs but read by no frozen fixture, as noted in its row. Validity and
...
Corrected again 2026-09-02 by TRUTH-01, for the opposite failure. The structural claim above was
fixed but the **population** figure beside it was not, so this document went on saying the edge had
"zero live pairs" for the rest of the day after the D-0023 enrichment had deliberately given it
7,950. Understating a result is not the safe direction of a documentation error. A presenter reading this
row would have talked past a working, populated feature, and `NEO4J_FIDELITY_AUDIT.md` - the audit
written to catch exactly this class of defect - was itself still carrying the superseded "does not
exist" verdict while this row carried the superseded population. Both are corrected now, and both
are recorded in `CLAIMS_AUDIT.md`.
